...
export default function TextOverlay({ text, index, onUpdate, onDelete }) {
  ...
  return (
    ...
    >
      {/* Delete button — outside overflow:hidden so it stays fully visible */}
      <button
        onClick={(e) => {
          e.stopPropagation();
          onDelete(index);
      ...
      <div style={{
        ...
        {editing ? (
          <textarea
            ref={inputRef}
            value={text.content || ''}
            onChange={(e) => onUpdate(index, { ...text, content: e.target.value })}
            onBlur={handleBlur}
